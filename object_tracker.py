...
lp_loc = os.getenv('locc')
print("LOC ID: ", lp_saving_dir)    
        


def upload_to_aws(lp_path, cropfilename, dir):
    ...
    s3imgpath = 'evidences/locations/abcloc/'+cropfilename

    s3.Bucket('licenseplatestorage').upload_file(Filename=lp_path, Key='evidences/locations/abcloc/'+cropfilename)
    print("Uploaded")
    print("Detecting License Plate number...")
    awsresponse = ""
    # print('License Plate number: ',getocr(awsresponse, dir))
    try:
        txtrct = boto3.resource(
        ...
        awsresponse = txtrct.detect_document_text(
            Document={
                'S3Object': {
                    'Bucket' : "licenseplatestorage",
                    'Name' : s3imgpath
                }
            })
...
def save_LP_captures(cropped_lp, original_vehicle, current_tracking_id, lp_saving_dir='./egLocationID/', lp_loc=""):
    ...
    cropfilename = str(current_tracking_id)+"_crop.jpg"
        ...
        for track in tracker.tracks:
            ...
            if lpcentY > 780 and lpcentY < 790:
                ...
                if current_tracking_id not in tracking_id_set:
                    ...
                    context = save_LP_captures(cropped_lp, original_vehicle, current_tracking_id, lp_saving_dir, lp_loc)
                    break
                    # print("======JSON======")
                    # print(context)
